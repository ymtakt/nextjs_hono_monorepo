...
import { ClientComponentLoading, Modal } from '@/components/functionless/general'
...
import { SearchBox } from '@/components/functionless/general/SearchBox'
import z from 'zod'
import { extractZodErrorMessage } from '@/utils/server-actions.ts'
...
export function TodoListClientPage(props: TodoListProps) {
  ...
  // 初期状態を作成
  const initialDeleteState: DeleteTodoActionState = createInitialActionState()
  // Server Actionをラップ
  const wrappedDeleteAction = useServerActionWrapper(deleteTodoAction, {
    onSuccess: ({ success }) => {
